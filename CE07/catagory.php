...
if (!$result) {
    ...
    if (mysqli_num_rows($result) == 0) {
        ...
        <td><h4><a href="catagory.php?cat=' . $row['idCategorie'] . '">' . $row['CategorieName'] . '</a><br><small>' . $row['CategorieDesc'] . '</small><h4></td>';
            echo '<td>';
            $topSql = "SELECT idTopic, TopicName FROM Library.Topic where CategoryFK = " 
                    . $row['idCategorie'] . " order by idTopic DESC limit 5";
            //echo $topSql;
...
if ($_GET['cat'] > 0) {
    ...
    $sql = "SELECT
            ...
            CategorieDesc
        FROM
            Categorie where idCategorie = $category";
//echo $sql;
    $result = $con->query($sql);
    if (!$result) die($conn->error);
    while ($row = $result->fetch_assoc()) {
        $catName = $row['CategorieName'];
        $catDes = $row['CategorieDesc'];
    }
